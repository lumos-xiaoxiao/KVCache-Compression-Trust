...
class Chat(ABC):
    ...
    #     """
    #     Load a Hugging Face model with the given arguments and apply modifications if needed.
    #     """
...
class HFChat(Chat):
    def __init__(self, model_name: str, conv_template: str, cache: str, disable_sys_prompt: None = False, **kwargs):
        ...
        try:
            ...
            # print("LOOK")
            # print(model_name)  # /data/maxiaoxiao/security/model/Llama-2-7b-chat-hf
            # TODO：修改
            # 替换路径
            new_model_name = model_name.replace("/data/maxiaoxiao/security/model", "huggingface")
            if "@" in model_name:  # model specified with a revision
                model_name = model_name[:model_name.find("@")]
            self.model_config = get_huggingface_model_config(new_model_name)
            print(e)
