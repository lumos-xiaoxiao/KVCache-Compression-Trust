        ...
        with htrack_block(f"Loading Hugging Face model for config {model_config}"):
            ...
            if quantization_config and quantization_config.model_loader == ModelLoader.AWQ:
                ...
                if (
                    ...
                    # TODO: 修改1
                    # max_capacity_prompts \ window_sizes \ 
                    
                    from KVCache.SparseKVCache.pyramidKV.pyramidkv.monkeypatch import replace_llama
                    replace_llama("pyramidkv")

                    config = AutoConfig.from_pretrained(model_name)
                    # 修改1
                    config.attn_implementation = "/data/maxiaoxiao/security/model/flash_attn-2.6.3+cu123torch2.4cxx11abiFALSE-cp38-cp38-linux_x86_64.whl"
                    config.device_map = "auto"
                    config.low_cpu_mem_usage = True
                    config.use_cache = True  # 如果需要启用缓存，可以取消注释
                    ...
                    attn_implementation="/data/maxiaoxiao/security/model/flash_attn-2.6.3+cu123torch2.4cxx11abiFALSE-cp38-cp38-linux_x86_64.whl",
                    device_map="auto",
